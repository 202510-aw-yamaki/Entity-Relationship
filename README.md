...
2. 最初の選択画面で、`空の状態から始める` `サンプルから始める` `JSON読込から始める` のいずれかを選びます。
3. 左カラムで箱や親箱を追加します。
4. 中央の図エリアで箱を動かし、線接続を作ります。
5. 右カラムで箱 / 線 / 箱内要件の詳細を編集します。
6. STEP1、STEP2、STEP3 を順に実行して不足や不整合を確認します。
7. 必要に応じて JSON 保存、JSON 読込、AI向け / human向け出力を使います。
8. AI に相談したいときは `AI相談JSON出力` を使います。
...
- 基本設定
  プロジェクト名、現在 STEP
- 操作
  箱追加、親箱追加、線接続開始 / 箱接続に戻る、削除、始め方を選び直す、Undo / Redo
- 保存と読込
...
- 開始時に選択画面が表示される
- 空の状態から始める、サンプルから始める、JSON読込から始める、の3通りが選べる
- 左 / 中央 / 右の3カラムが崩れない
...
- 起動直後は固定サンプルを自動表示せず、`空の状態から始める` `サンプルから始める` `JSON読込から始める` を選べるようにしました。作業中でも `始め方を選び直す` から開始方法を選び直せます。
