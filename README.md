...
## 追記: 最初のひな型づくりと今回の操作改善

- 選択中の箱 / 線は `Delete` キーでも削除できます。右カラムの入力欄や textarea に文字を入れている間は、図の削除は起きません。
- `親箱を追加` は、次の空き領域ドラッグで親箱を作るモードになりました。通常箱を親箱の中へ移動して離すと、その親箱の子要素として `parentId` が付きます。親箱の外へ十分に出して離すと解除されます。
- `JSON読込` は、途中保存した保存用JSONだけでなく、AI がこのHTML向けに作った保存用JSON互換のひな型も読めます。
- `AI相談JSON出力` は別用途です。こちらは AI に不足点や確認点を相談するための出力であり、`JSON読込` の対象ではありません。

ひな型づくりの流れ:

1. まだ案が曖昧でも、要望メモと `index.html` を AI に渡します。
2. `index.html` 内コメントにある「AIへ渡す定型文」を使い、保存用JSON互換を JSON のみで返してもらいます。
3. 返ってきた JSON を `JSON読込` で読み込んで、図の土台を出します。
4. その後に箱 / 親箱 / 要件 / 線を足して育て、詰まったら `AI相談JSON出力` を AI に渡して不足を埋めます。
